...
class EvaluationResult:
    ...
    def privacy_utility_ratio(self) -> float:
        ...
        if self.privacy.epsilon == 0:
            return 0.0
        return self.utility.f1_score / max(self.privacy.epsilon, 0.01)
...
def compute_utility_metrics(
    ...
    """Compute comprehensive utility metrics."""
    metrics = UtilityMetrics()
    metrics.accuracy = accuracy_score(y_true, y_pred)
    metrics.f1_score = f1_score(y_true, y_pred, average="binary", zero_division=0)
    metrics.precision = f1_score(y_true, y_pred, average="binary", zero_division=0)
    metrics.recall = f1_score(y_true, y_pred, average="binary", zero_division=0)

    # Recompute precision and recall properly
...
class ParetoFrontierAnalyzer:
    """
    Analyze the privacy-utility Pareto frontier across different
    privacy budgets and model configurations.
    """
    ...
    def add_result(
        ...
        """Add an evaluation result to the frontier analysis."""
        if config:
    ...
    def compute_pareto_frontier(
        ...
        privacy (lower epsilon) AND better utility (higher F1).
        """
        if not self.results:
            return []
        ...
        # Sort by privacy (lower epsilon = better privacy)
    ...
    def plot_pareto_frontier(
        ...
        config_names = [r.config.get("name", f"Config {i}") for i, r in enumerate(self.results)]
        ...
        # Plot all points
        ax.scatter(privacy_vals, utility_vals, c="steelblue", s=80, alpha=0.7, label="All configurations")
        ...
        pareto_points = sorted(
            zip([getattr(r.privacy, privacy_metric) for r in pareto],
                [getattr(r.utility, utility_metric) for r in pareto]),
            key=lambda x: x[0]
        )
        ...
        ax.set_xlabel(f"Privacy Budget (ε)" if privacy_metric == "epsilon" else privacy_metric)
    ...
    def plot_ablation_study(
        ...
        ax1 = axes[0]
        names = [r.config.get("name", f"Config {i}") for i, r in enumerate(self.results)]
        ...
        ax2 = axes[1]
        colors = ["green" if r.privacy.epsilon <= 3 else
                  "orange" if r.privacy.epsilon <= 8 else "red"
                  for r in self.results]
        sc = ax2.scatter(
        ...
        ax2.axvspan(0, 3, alpha=0.1, color="green", label="Strong Privacy (ε ≤ 3)")
        ax2.axvspan(3, 8, alpha=0.1, color="orange", label="Moderate Privacy (3 < ε ≤ 8)")
        ax2.axvspan(8, max([r.privacy.epsilon for r in self.results]) + 1,
                    alpha=0.1, color="red", label="Weak Privacy (ε > 8)")
    ...
    def save_results_table(self, save_name: str = "results_table.csv"):
        """Save results as a structured CSV table."""
        rows = []
        for r in self.results:
            row = {
                "config": r.config.get("name", "unknown"),
    ...
    def generate_report(self, save_name: str = "evaluation_report.json"):
