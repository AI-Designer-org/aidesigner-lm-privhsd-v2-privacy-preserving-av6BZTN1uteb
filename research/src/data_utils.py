...
class HateSpeechDataset(Dataset):
    ...
    Supports:
      ...
      - HateXplain
      - Privacy-augmented variants with synthetic author labels
    """
...
def create_author_labels(
    ...
    """
    Create synthetic author labels for adversarial disentanglement.

    In a privacy-preserving setting, we partition the dataset into
    pseudo-author groups based on stylistic features. This simulates
    the scenario where we want the model to be invariant to author identity.

    Uses a simple heuristic: hash-based partitioning on stylistic features
    (text length, punctuation density, capitalization ratio, etc.).

    Returns list of author IDs (0 to n_authors-1).
    """
    ...
    for text in texts:
        ...
        # Extract simple stylistic features for partitioning
        text_str = str(text)
...
def load_jigsaw_dataset(
    ...
    # Create author labels for disentanglement
...
def load_hatexplain_dataset(
    ...
    # HateXplain stores data as JSON with post_id as key
    ...
    if json_path.exists():
        ...
        for post_id, post_data in data.items():
            ...
            if label_votes:
                ...
                hate_labels.append(1 if majority < 2 else 0)  # hate/offensive → 1
            else:
...
def create_privacy_augmented_variant(
    ...
    This simulates data that has been pre-processed with:
      - Text sanitization (removing personal identifiers)
      - Stylistic perturbation (paraphrasing)
      - Noisy labels (simulating DP in the data curation process)

    Levels: 'low', 'medium', 'high' privacy perturbation.
    """
    rng = np.random.RandomState(random_seed)
    ...
    for text, label in zip(dataset.texts, dataset.hate_labels):
        ...
        # Word dropout noise (simulating stylized perturbation)
